...
# This file is copied and modified from cortx-motr:
# https://github.com/Seagate/cortx-motr/blob/main/scripts/addb-py/chronometry/addb2db.py
...
class mio_session(BaseModel):
    time  = IntegerField()
    pid = IntegerField()
    topic = TextField()
    session = IntegerField()

class mio_session_to_op(BaseModel):
    time  = IntegerField()
    pid = IntegerField()
    session = IntegerField()
    op = IntegerField()
...
class mio_op(BaseModel):
    time  = IntegerField()
    pid = IntegerField()
    topic = TextField()
    op = IntegerField()

class mio_op_to_motr_op(BaseModel):
    time  = IntegerField()
    pid = IntegerField()
    topic = TextField()
    session = IntegerField()
    mio_op = IntegerField()
    motr_op = IntegerField()
...
db_create_delete_tables = [client_to_dix, dix_to_mdix, dix_to_cas, cas_to_rpc,
                           ...
                           mio_session, mio_session_to_op, mio_op,
			   mio_op_to_motr_op]
...
class ADDB2PP:
    ...
    def mio_session(measurement, labels, table):
        time = measurement[1]
        topic = measurement[2]
        session = measurement[3]
        ret = {}
        ret["time"] = ADDB2PP.to_unix(time)
        ret["topic"] = topic
        ret["session"] = session
    ...
    def mio_session_to_op(measurement, labels, table):
        time = measurement[1]
        session = measurement[3]
        op = measurement[4]
        opcode = measurement[5]
        ret = {}
        ret["time"] = ADDB2PP.to_unix(time)
        ret["session"] = session
        ret["op"] = op
    ...
    def mio_op(measurement, labels, table):
        time = measurement[1]
        topic = measurement[2]
        op = measurement[3]
        ret = {}
    ...
    def mio_op_to_motr_op(measurement, labels, table):
        time = measurement[1]
        topic = measurement[2]
        session = measurement[3]
        mio_op = measurement[4]
        motr_op = measurement[5]
        ret = {}
        ret["time"] = ADDB2PP.to_unix(time)
        ret["topic"] = topic
        ret["session"] = session
    ...
    def __init__(self):
        self.parsers = {
            ...
            "mio-obj-open"      : (ADDB2PP.mio_session,  "mio_session"),
            "mio-obj-close"     : (ADDB2PP.mio_session,  "mio_session"),
            "mio-obj-op-init"   : (ADDB2PP.mio_session_to_op,     "mio_session_to_op"),
            "mio-op-fini"       : (ADDB2PP.mio_op,       "mio_op"),
            "mio-op-to-motr-io" : (ADDB2PP.mio_op_to_motr_op,     "mio_op_to_motr_op"),
            "mio-op-to-motr-kv" : (ADDB2PP.mio_op_to_motr_op,     "mio_op_to_motr_op"),
        }
